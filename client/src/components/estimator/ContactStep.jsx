...
export default function ContactStep({ contact, onChange, errors }) {
  const fieldError = (field) => errors?.find((e) => e.toLowerCase().startsWith(field));
  ...
  return (
    <div className="animate-riseIn space-y-4">
      ...
      <div>
        <label htmlFor="name" className="mb-1.5 block text-base font-semibold text-slate-900">
          Full name
        </label>
        <input
          id="name"
        ...
        />
        {fieldError('name') && <p className="mt-1 text-sm text-red-500">{fieldError('name')}</p>}
      </div>

      <div>
        <label htmlFor="phone" className="mb-1.5 block text-base font-semibold text-slate-900">
          Phone number
        </label>
        <input
          id="phone"
      ...
      <div>
        <label htmlFor="email" className="mb-1.5 block text-base font-semibold text-slate-900">
          Email address
        </label>
        <input
          id="email"
        ...
        {fieldError('email') || fieldError('a valid') ? (
          <p className="mt-1 text-sm text-red-500">{fieldError('email') || fieldError('a valid')}</p>
        ) : null}
      </div>
    </div>
  );
