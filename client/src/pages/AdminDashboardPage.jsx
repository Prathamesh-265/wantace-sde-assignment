    ...
    <div className="min-h-screen bg-canvas">
      <header className="border-b border-slate-200 bg-slate-900">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-5 py-5">
          <div>
            <p className="text-sm text-slate-400">Northline Roofing & Exteriors</p>
            <h1 className="text-xl font-bold text-white">Owner panel</h1>
          </div>
          <div className="flex items-center gap-4">
            <span className="hidden text-sm text-slate-400 sm:inline">{owner?.username}</span>
            <button
      ...
      <main className="mx-auto max-w-4xl px-5 py-6">
        {tab === 'config' && (
          <ConfigEditor onSaved={(v) => setToast(`Saved — now live as config v${v}.`)} />
        )}
